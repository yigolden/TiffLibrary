namespace TiffLibrary.Compression
{
    class CcittEncodingTable
    {
        ...
        {
            public Entry(ushort runLength, uint bits, ushort bitsRequired)
            {
                RunLength = runLength;
            ...
            public ushort BitsRequired { get; }
            public uint Bits { get; }
        }

        private const int TotalCodeCount = 104; // EOL code is not included
        private Entry[] _entries;
        private Entry _eolEntry;

        private CcittEncodingTable(bool isWhite)
        {
            _entries = new Entry[TotalCodeCount];
        ...
        {
            _entries[index] = new Entry(runLength, codeWord, codeLength);
            return index + 1;
        }

        private int AddEolCode(int index, ushort codeWord, byte codeLength)
        {
            _eolEntry = new Entry(0, codeWord, codeLength);
            return index;
        }

        public void EncodeRun(ref BitWriter2 writer, int length)
        {
            ...
            while (length >= 64)
            {
                // Encoding maximum code word
                if (length >= 2624)
                {
                    entry = entries[TotalCodeCount - 1];
                    writer.Write(entry.Bits, entry.BitsRequired);
                    length -= entry.RunLength;
                    continue;
                }

                // Encode using make-up codes
                for (int i = TotalCodeCount - 1; i >= 64; i--)
                {
                    entry = entries[i];
                    if (length >= entry.RunLength)
                    {
                        writer.Write(entry.Bits, entry.BitsRequired);
                        length -= entry.RunLength;
                        continue;
            ...
            // Encoding using terminating codes
            entry = entries[length];
            writer.Write(entry.Bits, entry.BitsRequired);
        }

        public void EncodeEndOfLine(ref BitWriter2 writer)
        {
            Entry eol = _eolEntry;
            writer.Write(eol.Bits, eol.BitsRequired);
        }
    }
}
