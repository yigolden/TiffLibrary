...
namespace TiffLibrary.PhotometricInterpreters
{
    ...
    /// </summary>
    public sealed class TiffChunkyRgbaAny32323232Interpreter : ITiffImageDecoderMiddleware
    {
        ...
        [CLSCompliant(false)]
        public TiffChunkyRgbaAny32323232Interpreter(bool isAlphaAssociated, bool undoColorPreMultiplying, TiffValueCollection<ushort> bitsPerSample, TiffFillOrder fillOrder = TiffFillOrder.HigherOrderBitsFirst)
        {
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                if (canDoFastPath)
                {
                    ...
                    for (int col = 0; col < cols; col++)
                    {
                        ...
                        pixel.A = (ushort)(FastExpandBits(bitReader.Read(bitsPerSample[3]), bitsPerSample[3], 32) >> 16);
                        pixelSpan[col] = pixel;
                    }
                }
                else
                {
                    ...
                    for (int col = 0; col < cols; col++)
                    {
                        ...
                        pixel.A = (ushort)(ExpandBits(bitReader.Read(bitsPerSample[3]), bitsPerSample[3], 32, reverseEndiannessA) >> 16);
                        pixelSpan[col] = pixel;
                    }
                }
