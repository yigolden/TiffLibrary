...
namespace TiffLibrary.PixelFormats.Converters
{
    ...
    internal class TiffBgra32ToGray16PixelConverter : TiffPixelConverter<TiffBgra32, TiffGray16>
    {
        public TiffBgra32ToGray16PixelConverter(ITiffPixelBufferWriter<TiffGray16> writer) : base(writer) { }

        public override void Convert(ReadOnlySpan<TiffBgra32> source, Span<TiffGray16> destination)
        {
        ...
        internal class Factory : ITiffPixelConverterFactory
        {
            public bool IsConvertible<TSource, TDestination>()
                ...
                => typeof(TSource) == typeof(TiffBgra32) && typeof(TDestination) == typeof(TiffGray16);
            public ITiffPixelBufferWriter<TSource> CreateConverter<TSource, TDestination>(ITiffPixelBufferWriter<TDestination> buffer)
                where TSource : unmanaged
                where TDestination : unmanaged
            {
                if (typeof(TSource) != typeof(TiffBgra32) || typeof(TDestination) != typeof(TiffGray16))
                {
                    throw new InvalidOperationException();
                }
                return Unsafe.As<ITiffPixelBufferWriter<TSource>>(new TiffBgra32ToGray16PixelConverter(Unsafe.As<ITiffPixelBufferWriter<TiffGray16>>(buffer)));
            }
        }

        internal class Converter : ITiffPixelSpanConverter<TiffBgra32, TiffGray16>
        {
            public void Convert(ReadOnlySpan<TiffBgra32> source, Span<TiffGray16> destination)
            {
