...
namespace TiffLibrary.PixelFormats.Converters
{
    ...
    internal class TiffBgra64ToBgra32PixelConverter : TiffPixelConverter<TiffBgra64, TiffBgra32>
    {
        public TiffBgra64ToBgra32PixelConverter(ITiffPixelBufferWriter<TiffBgra32> writer) : base(writer) { }

        public override void Convert(ReadOnlySpan<TiffBgra64> source, Span<TiffBgra32> destination)
        {
        ...
        internal class Factory : ITiffPixelConverterFactory
        {
            public bool IsConvertible<TSource, TDestination>()
                ...
                => typeof(TSource) == typeof(TiffBgra64) && typeof(TDestination) == typeof(TiffBgra32);
            public ITiffPixelBufferWriter<TSource> CreateConverter<TSource, TDestination>(ITiffPixelBufferWriter<TDestination> buffer)
                where TSource : unmanaged
                where TDestination : unmanaged
            {
                if (typeof(TSource) != typeof(TiffBgra64) || typeof(TDestination) != typeof(TiffBgra32))
                {
                    ThrowHelper.ThrowInvalidOperationException();
                }
                return Unsafe.As<ITiffPixelBufferWriter<TSource>>(new TiffBgra64ToBgra32PixelConverter(Unsafe.As<ITiffPixelBufferWriter<TiffBgra32>>(buffer)));
            }
        }

        internal class Converter : ITiffPixelSpanConverter<TiffBgra64, TiffBgra32>
        {
            public void Convert(ReadOnlySpan<TiffBgra64> source, Span<TiffBgra32> destination)
            {
