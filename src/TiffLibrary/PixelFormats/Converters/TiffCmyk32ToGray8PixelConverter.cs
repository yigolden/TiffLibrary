...
namespace TiffLibrary.PixelFormats.Converters
{
    ...
    internal class TiffCmyk32ToGray8PixelConverter : TiffPixelConverter<TiffCmyk32, TiffGray8>
    {
        public TiffCmyk32ToGray8PixelConverter(ITiffPixelBufferWriter<TiffGray8> writer) : base(writer) { }

        public override void Convert(ReadOnlySpan<TiffCmyk32> source, Span<TiffGray8> destination)
        {
        ...
        internal class Factory : ITiffPixelConverterFactory
        {
            public bool IsConvertible<TSource, TDestination>()
                ...
                => typeof(TSource) == typeof(TiffCmyk32) && typeof(TDestination) == typeof(TiffGray8);
            public ITiffPixelBufferWriter<TSource> CreateConverter<TSource, TDestination>(ITiffPixelBufferWriter<TDestination> buffer)
                where TSource : unmanaged
                where TDestination : unmanaged
            {
                if (typeof(TSource) != typeof(TiffCmyk32) || typeof(TDestination) != typeof(TiffGray8))
                {
                    ThrowHelper.ThrowInvalidOperationException();
                }
                return Unsafe.As<ITiffPixelBufferWriter<TSource>>(new TiffCmyk32ToGray8PixelConverter(Unsafe.As<ITiffPixelBufferWriter<TiffGray8>>(buffer)));
            }
        }

        internal class Converter : ITiffPixelSpanConverter<TiffCmyk32, TiffGray8>
        {
            ...
            public void Convert(ReadOnlySpan<TiffCmyk32> source, Span<TiffGray8> destination)
            {
