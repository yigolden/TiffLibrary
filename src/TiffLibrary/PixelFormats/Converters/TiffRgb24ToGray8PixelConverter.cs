...
namespace TiffLibrary.PixelFormats.Converters
{
    ...
    internal class TiffRgb24ToGray8PixelConverter : TiffPixelConverter<TiffRgb24, TiffGray8>
    {
        public TiffRgb24ToGray8PixelConverter(ITiffPixelBufferWriter<TiffGray8> writer) : base(writer) { }

        public override void Convert(ReadOnlySpan<TiffRgb24> source, Span<TiffGray8> destination)
        {
        ...
        internal class Factory : ITiffPixelConverterFactory
        {
            public bool IsConvertible<TSource, TDestination>()
                ...
                => typeof(TSource) == typeof(TiffRgb24) && typeof(TDestination) == typeof(TiffGray8);
            public ITiffPixelBufferWriter<TSource> CreateConverter<TSource, TDestination>(ITiffPixelBufferWriter<TDestination> buffer)
                where TSource : unmanaged
                where TDestination : unmanaged
            {
                if (typeof(TSource) != typeof(TiffRgb24) || typeof(TDestination) != typeof(TiffGray8))
                {
                    throw new InvalidOperationException();
                }
                return Unsafe.As<ITiffPixelBufferWriter<TSource>>(new TiffRgb24ToGray8PixelConverter(Unsafe.As<ITiffPixelBufferWriter<TiffGray8>>(buffer)));
            }
        }

        internal class Converter : ITiffPixelSpanConverter<TiffRgb24, TiffGray8>
        {
            public void Convert(ReadOnlySpan<TiffRgb24> source, Span<TiffGray8> destination)
            {
