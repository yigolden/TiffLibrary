...
namespace TiffLibrary.PixelFormats.Converters
{
    ...
    internal class TiffRgba64ToRgba32PixelConverter : TiffPixelConverter<TiffRgba64, TiffRgba32>
    {
        public TiffRgba64ToRgba32PixelConverter(ITiffPixelBufferWriter<TiffRgba32> writer) : base(writer) { }

        public override void Convert(ReadOnlySpan<TiffRgba64> source, Span<TiffRgba32> destination)
        {
        ...
        internal class Factory : ITiffPixelConverterFactory
        {
            public bool IsConvertible<TSource, TDestination>()
                ...
                => typeof(TSource) == typeof(TiffRgba64) && typeof(TDestination) == typeof(TiffRgba32);
            public ITiffPixelBufferWriter<TSource> CreateConverter<TSource, TDestination>(ITiffPixelBufferWriter<TDestination> buffer)
                where TSource : unmanaged
                where TDestination : unmanaged
            {
                if (typeof(TSource) != typeof(TiffRgba64) || typeof(TDestination) != typeof(TiffRgba32))
                {
                    throw new InvalidOperationException();
                }
                return Unsafe.As<ITiffPixelBufferWriter<TSource>>(new TiffRgba64ToRgba32PixelConverter(Unsafe.As<ITiffPixelBufferWriter<TiffRgba32>>(buffer)));
            }
        }

        internal class Converter : ITiffPixelSpanConverter<TiffRgba64, TiffRgba32>
        {
            public void Convert(ReadOnlySpan<TiffRgba64> source, Span<TiffRgba32> destination)
            {
