...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
    ...
    internal class Deflater
    {
        ...
        */
        ...
        {
            return Deflate(output.AsSpan(offset, length));
        }

        public int Deflate(Span<byte> output)
        {
            int offset = 0;
            ...
            for (; ; )
            {
                int count = pending.Flush(output.Slice(offset, length));
                offset += count;
                totalOut += count;
                length -= count;
